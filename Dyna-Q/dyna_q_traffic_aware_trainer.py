...
class DiscretizedRacetrack:
    ...
    def step(self, action_idx):
        ...
        if done:
            ...
            reward += 0.1  # mic bonus de miscare

            # === NOU: penalizare apropiere trafic ===
            for v in obs[1:]:
                vx, vy = v[0], v[1]
                dist = np.sqrt((vx - ex) ** 2 + (vy - ey) ** 2)
...
def antreneaza_si_salveaza():
    ...
    print(f">>> Încep antrenarea Dyna-Q ({EPISODES} episoade)...")
    ...
    for ep in range(EPISODES):
        ...
        if (ep + 1) % 100 == 0:
            print(
                f"Episod {ep+1}/{EPISODES} | Avg(20): {avg_score:.2f} | eps={agent.epsilon:.2f}"
            )
...
def ruleaza_demo():
    ...
    agent.load_model("model_traffic_aware.pkl")
    ...
    for i in range(5):
        ...
        while not done:
            ...
            total += reward
        print(f"Cursa {i+1} | Scor: {total:.1f}")
